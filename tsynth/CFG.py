...
class CFG:
    def __init__(self, rules):
        self.rules = defaultdict(list, rules)
...
class Word_Generator:
        def __init__(self, cfg, logic, variables, constants):
            self.memory = defaultdict(defaultdict)
        ...
        def generate(self, from_symbol, word_length):
            if word_length in self.memory[from_symbol]:
                return self.memory[from_symbol][word_length]

            elif word_length == 1:
                res = list(flatten(filter(lambda l: len(l) == 1, self.cfg.rules[from_symbol])))
                self.memory[from_symbol][word_length] = res
                return res
                
            else:
                res = []
                for nt1, nt2 in filter(lambda l: len(l) == 2, self.cfg.rules[from_symbol]):
