...
class CFG:
    ...
    def convert_to_chomsky_normal_form(self):
                    ...
                    for j, sym in enumerate(rule):
                        if sym not in new_grammar.non_terminals():
                            if sym not in changed_terminals:
                                new_non_terminal = new_grammar._find_symbol_not_in_grammar(
                                    "N"
                                )
        ...
        for nt in new_grammar.non_terminals():
            for i, rule in enumerate(new_grammar.rules[nt]):
                if len(rule) > 2:
                    new_non_terminals = new_grammar._find_multiple_symbols_not_in_grammar(
                        "A", len(rule) - 2
                    )
                    ...
                    for j in range(len(new_non_terminals[0:-1])):
                        new_grammar.add_rule(
                            new_non_terminals[j],
                            [rule[j + 1], new_non_terminals[j + 1]],
                        )
...
class Word_Generator:
    def __init__(self, cfg, spec, synth_func):
        ...
        for nt in self.cfg.rules:
            self.unit_productions[nt] = list(
                flatten(filter(lambda l: len(l) == 1, self.cfg.rules[nt]))
            )

        self.product_productions = {}
        for nt in self.cfg.rules:
            self.product_productions[nt] = list(
                filter(lambda l: len(l) == 2, self.cfg.rules[nt])
            )
        ...
        if word_length == 1:
            ...
            for nt1, nt2 in self.product_productions[from_symbol]:
                for i in range(1, word_length):
                    ...
                    words = [
                        "{} {}".format(w1, w2) for w1 in nt1_words for w2 in nt2_words
                    ]
                    res.extend(words)
